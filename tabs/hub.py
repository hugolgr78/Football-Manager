...
class Hub(ctk.CTkFrame):
    def __init__(self, parent, session, manager_id):
        ...
        self.nextMatch = nextMatch(self, self.session, self.manager_id)
        self.playerMorale = PlayerMorale(self, self.session, self.manager_id)

        self.leagueTable = LeagueTableScrollable(self, 490, 312, 0, 0.3, TKINTER_BACKGROUND, DARK_GREY, GREY_BACKGROUND, "nw", small = True)
        self.leagueTable.defineManager(self.session, self.manager_id)
        self.leagueTable.addLeagueTable()

        self.next5Matches = next5Matches(self, self.session, self.manager_id, TKINTER_BACKGROUND, 333, 600, 90, 0.67, 0.3, "nw", 0.4, self)
    ...
    def openClubProfile(self, team):
        ...
        self.profile = TeamProfile(self.parent, self.session, team.manager_id, self.changeBack)
...
class nextMatch(ctk.CTkFrame):
    ...
    def showNextMatch(self, replace = False):
        ...
        nextMatch = Matches.get_team_next_match(self.session, self.parent.team.id, self.parent.league.league_id)

        homeTeam = Teams.get_team_by_id(self.session, nextMatch.home_id)
        awayTeam = Teams.get_team_by_id(self.session, nextMatch.away_id) 

        homeImage = Image.open(io.BytesIO(homeTeam.logo))
        homeImage.thumbnail((150, 150))
        homeLogo = TeamLogo(self, self.session, homeImage, homeTeam, TKINTER_BACKGROUND, 0.35, 0.5, "center", self.parent)

        awayImage = Image.open(io.BytesIO(awayTeam.logo))
        awayImage.thumbnail((150, 150))
        awayLogo = TeamLogo(self, self.session, awayImage, awayTeam, TKINTER_BACKGROUND, 0.65, 0.5, "center", self.parent)
...
class PlayerMorale(ctk.CTkScrollableFrame):
    ...
    def addPlayerMorale(self, replace = False):
        ...
        players = Players.get_all_players_by_team(self.session, self.parent.team.id)
